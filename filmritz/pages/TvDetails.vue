<template>
  <div class="min-h-screen text-white py-10 px-6 md:px-10 mt-20">
    ...
    <div v-else>
      <div v-if="latestSeason" class="relative h-[70vh] mb-10 rounded-2xl overflow-hidden" :style="heroBgStyle">
        ...
        <div class="absolute bottom-0 left-0 p-6 md:p-10 max-w-3xl space-y-3 animate-fade-up">
          ...
          <NuxtLink :to="`/ng/tv/${slugify(tv.name)}-${tv.id}`"
              ...
              <DialogPanel v-if="selectedEpisode"
                ...
                <div v-else class="overflow-y-auto h-[90vh] relative">
                  ...
                  <div
                    ...
                    <DialogTitle as="h2" class="text-3xl font-[Gilroy-Bold]">{{ selectedEpisode.name }}</DialogTitle>
                    ...
                    <div v-if="episodeCredits.length" class="mt-8">
                      ...
                      <div class="grid grid-cols-2 gap-4">
                        <div v-for="actor in episodeCredits" :key="actor.id"
                          ...
                          <img
                            :src="actor.profile_path ? 'https://image.tmdb.org/t/p/w185' + actor.profile_path : 'https://placehold.co/80x80/0f0f0f/FF0000?text=NO+IMG'"
                            class="w-14 h-14 rounded-full object-cover" />
                          <div>
                            <p class="font-[Gilroy-Medium] text-base">{{ actor.name }}</p>
                            <p class="text-sm text-gray-400 font-[Gilroy-Regular]">as {{ actor.character || 'Unknown Role' }}</p>
                          </div>
                        </div>
                      </div>
                    </div>
...
import { getTVDetails, getTVSeasonDetails, getTVEpisodeDetails, getTVEpisodeCredits } from '../../src/api/tmdb'
...
const heroBgStyle = computed(() => {
  ...
  return path
    ? { backgroundImage: `url(https://image.tmdb.org/t/p/original${path})`, backgroundSize: 'cover', backgroundPosition: 'center' }
    : { backgroundImage: 'url(https://placehold.co/300x450/0f0f0f/FF0000?text=FILMRITZ%0ANO+IMAGE)' }
})

const episodeBgStyle = computed(() => {
  const path = selectedEpisode.value?.still_path
  return path
    ? { backgroundImage: `url(https://image.tmdb.org/t/p/w1280${path})`, backgroundSize: 'cover', backgroundPosition: 'center' }
    : { backgroundImage: 'url(https://placehold.co/600x400/0f0f0f/FF0000?text=FILMRITZ%0ANO+IMAGE)' }
})
...
async function openModal(ep: Episode) {
  ...
  selectedEpisode.value = await getTVEpisodeDetails(tv.value.id, ep.season_number, ep.episode_number)
...
// -----------------------------
const pageTitle = computed(() => `FilmRitz — ${tv.value?.name || 'TV Show Details'}`)
const pageDescription = computed(() => tv.value?.overview || 'Discover your favorite TV shows on FilmRitz')
const pageUrl = computed(() => `https://filmritz.vercel.app/tv/${slugify(tv.value?.name)}-${tv.value?.id}`)
...
useHead(() => ({
  ...
  meta: [
    ...
    { property: 'og:image', content: tv.value?.backdrop_path ? `https://image.tmdb.org/t/p/w780${tv.value.backdrop_path}` : 'https://filmritz.vercel.app/filmritzlogo2.png' },
    { name: 'twitter:title', content: pageTitle.value },
    { name: 'twitter:description', content: pageDescription.value },
    { name: 'twitter:image', content: tv.value?.backdrop_path ? `https://image.tmdb.org/t/p/w780${tv.value.backdrop_path}` : 'https://filmritz.vercel.app/filmritzlogo2.png' },
  ...
  script: [
    {
      ...
      innerHTML: JSON.stringify({
        ...
        url: pageUrl.value,
        image: tv.value?.backdrop_path ? `https://image.tmdb.org/t/p/w780${tv.value.backdrop_path}` : 'https://filmritz.vercel.app/filmritzlogo2.png',
      }),
    } as any
  ]
}))
