<template>
  <div
    class="min-h-screen text-white py-10 mt-18 transition-all duration-900 animate-fade-up"
  >
    <!-- Hero Section -->
    <div class="px-6 md:px-10 mb-6">
      <div
        class="bg-gray-800 rounded-lg p-6 flex flex-col md:flex-row items-center justify-between gap-6"
      >
        <!-- Title & Description -->
        <div class="flex flex-col items-center text-center">
          ...
          <p class="mt-2 text-white font-[Gilroy-Medium] max-w-xl">
            Explore the trending and top-rated
            {{ isMoviePage ? "movies" : "TV shows" }}.
          </p>
        </div>
    ...
    <div
      v-else
      class="mt-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 px-6 md:px-10"
    >
      <div
        v-for="item in filteredMedia"
        :key="item.id + item.media_type"
    ...
    <div class="flex justify-center mt-8 gap-2">
      ...
      <span class="px-3 py-1 bg-gray-900 rounded">{{
  ...
  script: [
    {
      ...
      innerHTML: JSON.stringify({
        ...
        name: isMoviePage.value
          ? "FilmRitz Movies Collection"
          : "FilmRitz TV Shows Collection",
...
watch([appliedFilters, selectedCategory], () => {
  useHead({
    ...
    meta: [
      {
        name: "description",
        content: `${pageDescription.value} - Filtered by ${selectedCategory.value}`,
      },
    ],
  });
});
