<template lang="html">
  <div class="relative">
    <div class="absolute inset-y-0 end-0 flex items-center pe-3.5 cursor-pointer" v-if="showHide"
      @click="toggleVisibility">
      <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
        fill="currentColor" viewBox="0 0 24 24" v-if="!show">
      ...
      <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
        fill="currentColor" viewBox="0 0 24 24" v-else>
        <path fill-rule="evenodd"
    ...
    <input :type="show ? 'text' : 'password'" :name="name" :id="id" :placeholder="placeholder"
      :class="error ? formClass.text.authError : formClass.text.auth" required v-model="input" />
  </div>
</template>
