<template>
  <div v-if="!isLoggedIn"
    ...
    <span>
      ...
      <div v-else class="absolute inset-0 bg-[linear-gradient(to_bottom,#000000CC_10%,#00000099_50%,#000000_100%)]"></div>
    </span>
    ...
    <div
      ...
      <div class="items-center flex flex-col gap-3">
        ...
        <h3 class="font-[Gilroy-Medium] max-w-96 md:max-w-full">
          Ready to explore? Enter your email to discover movies and shows tailored to you.
        </h3>
      </div>
  ...
  <div v-else class="relative w-full">
    ...
    <div v-if="loading"
      ...
      <div class="absolute inset-0 bg-linear-to-b from-gray-800/80 via-gray-900/70 to-black/90"></div>
      <div class="z-10 flex flex-col gap-4 transition-all duration-900 animate-fade-up">
    ...
    <!-- Error State -->
    <div v-else-if="error" class="text-red-500 text-center py-20 bg-red-500/10 rounded-xl">
      <p class="font-[Gilroy-SemiBold] text-xl mb-4">{{ error }}</p>
      <button @click="retryLoad"
        class="px-6 py-3 bg-[#b20710] hover:bg-[#e32125] text-white rounded-lg font-[Gilroy-SemiBold] transition-all">
        Retry
      </button>
    </div>
    ...
    <div v-else class="relative h-screen overflow-hidden">
      ...
      <!-- Slides Container -->
      <div v-for="(item, index) in trendingAll.slice(0, 7)" :key="item.id"
        class="absolute inset-0 transition-all duration-1000"
        :class="currentSlideIndex === index ? 'opacity-100 z-10' : 'opacity-0 z-0'"
        @mouseenter="handleSlideHover(item, index)">
        <!-- Background Image -->
        <div class="absolute inset-0 bg-fixed bg-center bg-cover bg-no-repeat transition-all duration-700" :style="{
        ...
        <div class="absolute top-0 left-0 right-0 h-20 bg-linear-to-b from-black/90 to-transparent z-20"></div>
        <div class="absolute bottom-0 left-0 right-0 h-64 bg-linear-to-t from-black via-black/60 to-transparent z-20">
        </div>

        <!-- Content Overlay -->
        <div class="absolute inset-0 flex flex-col justify-end p-6 md:p-12 lg:p-16 z-30">
          ...
          <!-- Metadata Row -->
          <div class="flex flex-wrap items-center gap-3 md:gap-4 text-[#ffffffec] mb-4 animate-fade-up"
            style="animation-delay: 0.1s">
            <!-- Rating -->
            <div class="flex items-center gap-2 px-3 py-1.5 bg-[#b20710] rounded-lg">
              <span class="text-yellow-400">⭐</span>
              <span class="font-[Gilroy-Bold]">{{ item?.vote_average?.toFixed(1) }}</span>
            </div>

            <!-- Media Type -->
            <span class="px-3 py-1.5 bg-white/20 backdrop-blur-sm rounded-lg font-[Gilroy-SemiBold] text-sm">
            ...
            <div class="hidden md:flex gap-2">
              <router-link v-for="genreName in getGenreNames(item.genre_ids).slice(0, 3)" :key="genreName"
          ...
          <!-- Genres (Mobile) -->
          <div class="flex md:hidden gap-2 mb-4 flex-wrap animate-fade-up" style="animation-delay: 0.15s">
            <span v-for="genreName in getGenreNames(item.genre_ids).slice(0, 2)" :key="genreName"
          ...
          <!-- Overview -->
          <p class="text-base md:text-lg text-gray-200 font-[Gilroy-Medium] line-clamp-3 max-w-3xl mb-6 animate-fade-up"
            style="animation-delay: 0.2s">
            {{ item.overview }}
          </p>

          <!-- Action Buttons -->
          <div class="flex flex-wrap gap-3 md:gap-4 animate-fade-up" style="animation-delay: 0.25s">
            <button
            ...
            <button @click="modalStore.open(item.media_type, { id: item.id })"
      ...
      <div class="absolute bottom-0 left-0 right-0 h-1 bg-white/20 z-40">
        <div class="h-full bg-[#b20710] transition-all duration-100" :style="{ width: `${progressPercentage}%` }"></div>
      </div>

      <!-- Navigation Dots -->
      <div class="absolute bottom-2 md:bottom-8 left-1/2 transform -translate-x-1/2 flex gap-2 z-40">
        <button v-for="(item, index) in trendingAll.slice(0, 7)" :key="index" @click="goToSlide(index)"
          class="transition-all duration-300 rounded-full" :class="currentSlideIndex === index
            ? 'w-8 h-2 bg-white'
            : 'w-2 h-2 bg-white/40 hover:bg-white/60'"></button>
      </div>

      <!-- Scroll Indicator -->
      <div class="absolute bottom-20 left-1/2 transform -translate-x-1/2 animate-bounce z-40 hidden md:block">
