      ...
      <div class="fixed inset-0 flex items-center justify-center p-4">
        ...
        <DialogPanel class="relative w-full max-w-4xl h-[40em] overflow-hidden rounded-xl text-white shadow-xl">
          ...
          <button @click="modalStore.closeMovie"
            ...
            <svg viewBox="0 0 36 36" width="36" height="36" data-icon="PlusLarge" data-icon-id=":r9:" aria-hidden="true"
              class="tramsform -rotate-45" xmlns="http://www.w3.org/2000/svg" fill="none" role="img">
          ...
          <div class="absolute bottom-0 left-0 z-10 p-6 space-y-3">
            <div class="">
              <h2 class="text-6xl font-[Gilroy-Bold]">{{ movie?.title }}</h2>
              <span class="px-2 py-1 bg-green-700 text-green-100 rounded-md text-sm font-[Gilroy-SemiBold]">{{
                movie?.vote_average.toFixed(1) }}</span>
              <span class="ml-4 text-sm font-[Gilroy-Medium]">{{ new Date(movie?.release_date).getFullYear() }}</span>
            </div>
            <p class="mt-3 max-w-2xl text-xl font-[Gilroy-SemiBold]">{{ movie?.overview }}</p>
            <button
