...
<template>
  <section class="bg-black transition-all duration-900 animate-fade-up relative z-10 overflow-hidden">
      ...
      <Swiper :modules="[Navigation]" :slidesPerGroup="4" :loop="false"
        ...
        <SwiperSlide v-for="(item, index) in top10" :key="item.id">
          <div
            ...
            <img :src="`https://image.tmdb.org/t/p/w500${item.backdrop_path}`" @click="openModal(item)"
              sizes="(max-width: 640px) 300px, (max-width: 1024px) 500px, 780px" :alt="item.title" loading="lazy"
    ...
    <div
      class="custom-prev hidden absolute left-0 top-1/2 -translate-y-1/2 w-10 h-96 md:flex items-center justify-center bg-black text-white cursor-pointer transition-all duration-500 pr-2"
      :class="{
...
const top10 = computed(() => {
  ...
  // default is movie
  return topRated.value.slice(0, 10);
});
...
onMounted(async () => {
  loading.value = true;
  try {
    if (props.mediaType === "tv") await getTopRatedTV();
    else await getTopRated();
  } catch (err: any) {
