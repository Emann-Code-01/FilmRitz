...
<template>
  ...
  <TransitionRoot :show="modalStore.isModalOpen" as="template" v-else>
    <Dialog @close="modalStore.closeModal" class="relative z-50 transition-all duration-200">
      <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-[linear-gradient(to_bottom,#000000CC_10%,#000000_50%)]"></div>
        <button class="sr-only" tabindex="-1" aria-hidden="true">hidden focus target</button>
        <DialogPanel v-if="media"
          ...
          <button ref="initialFocus" @click="modalStore.closeModal"
            class="hover:bg-[#b20710]/70 absolute top-5 right-5 rounded-full p-1 cursor-pointer transition-all duration-300 outline-none">
        ...
        <!-- ✅ REPLACED "Loading details..." WITH SKELETON -->
        <DialogPanel v-else
