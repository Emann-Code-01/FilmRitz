...
<template>
  <div v-if="loading" class="">
    <div class="fixed inset-0 flex items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-4xl h-[40em] overflow-hidden rounded-xl text-white shadow-xl">
        <div
          class="absolute inset-0 bg-fixed bg-center bg-cover mx-auto bg-no-repeat transition-all duration-500 animate-fade-up"
          :style="{
            backgroundImage: 'url(https://placehold.co/300x450/0f0f0f/FF0000?text=FILMRITZ%0ALOADING+IMAGE&font=montserrat)',
          }"></div>
      ...
      <!-- <div class="absolute bottom-0 left-0 z-10 p-6 space-y-3">
        <DialogTitle as="h2" class="md:text-6xl text-3xl font-[Gilroy-Bold]"></DialogTitle>
  ...
  <TransitionRoot :show="modalStore.isModalOpen" as="template" v-else>
    <Dialog @close="modalStore.closeModal" class="relative z-50 transition-all duration-200">
      <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-[linear-gradient(to_bottom,#000000CC_10%,#000000_50%)]"></div>
        <DialogPanel v-if="media"
          ...
          <button ref="initialFocus" @click="modalStore.closeModal"
          ...
          <div class="absolute bottom-0 left-0 z-10 p-6 space-y-3">
            <DialogTitle as="h2" class="md:text-6xl text-3xl font-[Gilroy-Bold]">{{ media?.title }}</DialogTitle>
        ...
        <DialogPanel v-else class="flex items-center justify-center h-[40em] text-white">Loading details...
        </DialogPanel>
      </div>
    </Dialog>
  </TransitionRoot>
