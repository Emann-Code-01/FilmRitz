  ...
  <section class="relative z-10">
    ...
    <div v-else class="upcoming-grid">
      <div class="grid grid-cols-12 gap-4 auto-rows-auto">
        <div
          v-if="mediaList[0]"
          class="col-span-12 md:col-span-6 lg:col-span-6 row-span-2 relative group cursor-pointer overflow-hidden rounded-xl transition-all duration-500 hover:shadow-2xl ease-in-out hover:shadow-[#b20710]/30"
        ...
        >
          ...
          >
            ...
            <p
              class="text-gray-300 text-base font-[Gilroy-Regular] line-clamp-3 mb-4 max-w-2xl lg:opacity-0 lg:translate-y-5 lg:group-hover:opacity-100 lg:group-hover:translate-y-0 transition-all duration-500 lg:group-hover:animate-fade-up delay-100"
            >
...
import { getRotatedTopRated } from "@/services/mediaRotation";
...
const loadTopRatedMedia = async () => {
  loading.value = true;
  error.value = null;
  try {
    // Fetch 3 pages (60+ items) and display top 20
    // Use rotated top-rated for variety (cached for 6 hours)
    mediaList.value = await getRotatedTopRated();
  } catch (err: any) {
    console.error("❌ Failed to fetch top rated media:", err);
    error.value =
      "Couldn't load top rated movies and shows. Please refresh in a bit.";
  } finally {
    loading.value = false;
  }
};
