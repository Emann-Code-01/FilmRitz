  ...
  <div class="min-h-screen bg-[#0a0a0a] text-white pb-20">
        ...
        <div class="flex items-center justify-between">
          <div class="grid items-center gap-4 md:hidden">
            ...
            <button v-if="history.length" @click="clearHistory"
              class="px-3 md:px-6 py-2 md:py-2.5 bg-red-600/20 border border-red-600/50 text-red-400 rounded-xl font-[Gilroy-SemiBold] hover:bg-red-600 hover:text-white transition-all cursor-pointer">
              Clear History
            </button>
          </div>
          <div class="hidden md:flex items-center justify-between gap-4 w-full">
            ...
            <button v-if="history.length" @click="clearHistory"
              class="px-3 md:px-6 py-2 md:py-2.5 bg-red-600/20 border border-red-600/50 text-red-400 rounded-xl font-[Gilroy-SemiBold] hover:bg-red-600 hover:text-white transition-all cursor-pointer items-end">
              Clear History
            </button>
          </div>
        </div>
    ...
    <div class="px-6 md:px-10 max-w-[1230px] lg:max-w-[1440px] mx-auto">
      <div v-if="loading" class="space-y-4">
        <div v-for="n in 10" :key="n" class="flex gap-4 bg-gray-800/50 rounded-2xl p-4 animate-pulse">
      ...
      <div v-else class="space-y-4">
        <div v-for="item in history" :key="item.id" @click="goToMedia(item)"
          ...
          <div class="w-32 shrink-0">
            <img :src="getPoster(item.poster_path)" class="w-full h-48 object-cover rounded-xl" />
          </div>

          <div class="flex-1">
            <h3 class="text-2xl font-[Gilroy-Bold] mb-2 group-hover:text-[#b20710] transition-colors">
              {{ item.title || item.name }}
            </h3>

            <div class="flex flex-wrap items-center gap-3 mb-3 text-sm">
              <span class="px-3 py-1 bg-[#b20710] rounded-lg font-[Gilroy-SemiBold]">
          ...
          <button @click.stop="removeFromHistory(item.id)"
            class="w-10 h-10 rounded-full bg-white/5 hover:bg-red-600 flex items-center justify-center transition-all cursor-pointer">
